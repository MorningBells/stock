...
DB_IS_RUN=`docker ps --filter "name=stockdb" --filter "status=running" | wc -l `
if [ $DB_IS_RUN -lt 2 ]; then

    #判断文件夹存在不。
    if [ ! -d "${PWD}/data/stockdb/data" ]; then
        mkdir -p ${PWD}/data/stockdb/data
    fi
    ...
    if [ $DB_IS_RUN -ne 2 ]; then
        ...
        ####################### 创建数据库 #######################
        docker run --name stockdb -v ${PWD}/data/stockdb/data:/var/lib/mysql --restart=always \
        -e MYSQL_ROOT_PASSWORD=stockdb -e MYSQL_DATABASE=stock_data -e TZ=Asia/Shanghai \
        -p 13307:13307 -d mysql:5.7 --character-set-server=utf8mb4 --collation-server=utf8mb4_unicode_ci
        echo "starting stockdb ..."
        echo "wait 30 second , stockdb is starting ."
        sleep 30
    else
        echo "stockdb is running !!!"
    fi


    #检查stockdb是否启动，等待5秒钟，再次检查stockdb启动
    DB_IS_RUN=`docker ps --filter "name=stockdb" --filter "status=running" | wc -l `
    if [ $DB_IS_RUN -ne 2 ]; then
        echo "stockdb is not running !!!"
        exit 1;
    fi
fi
