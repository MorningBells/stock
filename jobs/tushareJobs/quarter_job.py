...
def stat_all(tmp_datetime):
    ...
    data = ak.stock_em_yjbb()
    # 增加季度字段。
    data = concat_quarter(year, quarter, data)
    # 处理重复数据，保存最新一条数据。最后一步处理，否则concat有问题。
    # data = data.drop_duplicates(subset="code", keep="last")
    data = data.drop_duplicates(subset="股票代码", keep="last")
    # 插入数据库。
    common.insert_db(data, "ts_report_data", False, "`quarter`,`股票代码`")
    ...
    # 插入数据库。
    common.insert_db(data, "ts_profit_data", False, "`quarter`,`代码`")
    ...
    data = ak.stock_em_xjll()
    # 增加季度字段。
    data = concat_quarter(year, quarter, data)
    # 处理重复数据，保存最新一条数据。最后一步处理，否则concat有问题。
    data = data.drop_duplicates(subset="股票代码", keep="last")
    # 插入数据库。
    common.insert_db(data, "ts_cashflow_data", False, "`quarter`,`股票代码`")
